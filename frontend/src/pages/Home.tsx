...
const Home = (): React.JSX.Element => {
  ...
  useEffect(() => {
    loadTeams().then(() => setLoading(false)).catch(console.error)
  ...
  return <>
    <Message icon color='green'>
      ...
      <Message.Content>
        ...
        Thank you to everyone who participated and congratulations to the winners! View the full standings for the
        Blue division <Link to={'/blue/standings'} className='banner-link'>here</Link> and the Gold division <Link
        to={'/gold/standings'} className='banner-link'>here</Link>.
      </Message.Content>
    </Message>
    <Countdown />
    <Block size="xs-12">
    ...
    <Block size="xs-12">
      ...
      {isLoading ? (
        ...
        <Table celled>
          ...
          <Table.Body>
            {sortedTeams?.map(team => <TeamRow key={`${team.school_name}-${team.team_name}`} team={team} />)}
          </Table.Body>
        </Table>
      )}
    </Block>
...
const TeamRow = ({ team }: { team: TeamType }) => {
  return <Table.Row>
