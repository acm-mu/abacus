  ...
  return (
    <Switch>
      ...
      <Route
        exact
        path="/blue/practice"
        component={() => <PracticeProblems problems={problems} submissions={submissions} />}
      />
      <Route
        path="/blue/practice/:id"
        component={() => {
          const { id } = useParams<{ id: string }>()

          if (id in submissions) return <PracticeSubmission submission={submissions[id]} />
          else if (id in problems)
            return (
              <PracticeProblem submissions={Object.values(submissions).filter((submission) => submission.pid == id)} />
            )
