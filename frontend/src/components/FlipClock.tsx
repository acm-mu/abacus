...
const monthsWithThirtyOneDays = [0, 2, 4, 6, 7, 9, 11] // January, March, May, July, August, October, December
...
function timeRemaining(endTime: Date) {
  ...
  if (months > 0) {
    ...
    for (let i = 0; i < months; i++) {
      ...
      if (month == 1) deltaDays += 28
      // February
      else if (monthsWithThirtyOneDays.includes(month)) deltaDays += 31
      else deltaDays += 30
      month = (month + 1) % 12
    }
...
const FlipClock = (props: { count_to: Date }): React.JSX.Element => {
  ...
  return <div className="flip-clock">
    {Object.entries(timeRemaining(props.count_to)).map(([label, value]) => {
      if (label == 'Total') {
        return <React.Fragment key={`fp-${label}`}></React.Fragment>
      }
      const val = ('0' + value).slice(-2)

      return <React.Fragment key={`fp-${label}`}>
          <span
